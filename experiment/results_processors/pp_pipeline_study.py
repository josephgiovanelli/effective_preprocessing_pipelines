...
def main():
    ...
    evaluation_results_path = os.path.join(results_path, "evaluation2_3")
    plots_path = create_directory(plots_path, "extension")
    input_auto = os.path.join(evaluation_results_path, "pipeline_algorithm")
    ...
    results_map.to_csv(os.path.join(new_results_path, "pp_pipeline_operator_study.csv"), index=False)
        ...
        for operator in result.index.get_level_values('operator').unique():
            ...
            if flag:
                ...
                for algo in ["nb", "knn", "rf"]:
                    try:
                        c = result.loc[transformation, operator, algo].to_numpy().flatten()[0]
                    except:
                        c = 0
                    curr_bar.append(c)
                if transformation != last_transformation or last_transformation == '':
                    ...
                    if not(cumulative):
                        ...
                        for algo in ["nb", "knn", "rf"]:
                            try:
                                c = result.loc[last_transformation, last_operator, algo].to_numpy().flatten()[0]
                            except:
                                c = 0
                            last_bar.append(c)
    ...
    fig.savefig(os.path.join(plots_path, "pp_pipeline_study_grouped.pdf"), bbox_extra_artists=(lgd,text), bbox_inches='tight')
