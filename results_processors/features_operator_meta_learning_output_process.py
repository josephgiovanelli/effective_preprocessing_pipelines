...
def main():
    ...
    input, result_path = "results/summary/evaluation2_3", "results/summary/evaluation2_3"
    input_auto = "results/evaluation2_3/pipeline_algorithm"
    result_path = create_directory(result_path, "extension")
    algorithms = ["knn", "nb", "rf"]

    ## Meta-features Loading
    all_classification = pd.read_csv('results_processors/meta_features/extended_meta_features_all_classification.csv')
    openml_cc_18 = pd.read_csv('results_processors/meta_features/extended_meta_features_openml_cc_18.csv')
    study_1 = pd.read_csv('results_processors/meta_features/extended_meta_features_study_1.csv')
            ...
            if acronym in results:
                with open(os.path.join(input_auto, acronym + '.json')) as json_file:
                    ...
                    if features_flag == "FeatureUnion":
                        results_map.loc[results_map["leaf"] == leaf, "SelectKBest"] += 1
                        results_map.loc[results_map["leaf"] == leaf, "PCA"] += 1
                    else:
